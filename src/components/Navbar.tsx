...
export default function Navbar(){
    ...
    return(
        <div className={isOpen ? "flex xl:flex-row flex-col xl:space-y-0 space-y-5 fixed justify-between items-center h-auto xl:p-5 xl:py-5 py-1 xl:px-4 px-2 top-0 xl:w-full w-full z-50 duration-100 bg-black/0 transition-all" : "flex xl:flex-row flex-col xl:space-y-0 space-y-7 fixed items-center transition-all h-screen xl:p-5 py-5 px-4 top-0 xl:w-9/12 w-full z-40 justify-start backdrop-blur-xl bg-black/50 duration-100"}>
            ...
            <ul className={isOpen ? "xl:flex xl:flex-row flex-col gap-14 text-neutral-400 backdrop-blur-lg hidden border border-neutral-700/50 rounded-xl text-center px-20 py-2" : "flex xl:flex-row flex-col text-center space-y-5 text-xl xl:space-x-14 text-neutral-300"}>
                <li className="hover:text-white duration-100 xl:hidden hover:bg-neutral-600"><Link href={"/"} onClick={()=> setOpen(!isOpen)}>Home</Link></li>
                <li className="hover:text-white duration-100 hidden xl:flex hover:bg-neutral-700 px-2 rounded-xl"><Link href={"/"} className="inline-flex justify-center items-center gap-2">Home <TiHome /></Link></li>
                <li className="hover:text-white duration-100 hover:bg-neutral-700 px-2 rounded-xl"><a className="inline-flex justify-center items-center gap-2" download={"CV_DANTE_RIVAROLA_DINATALE_ENGLISH"} href={"/CV_DANTE_RIVAROLA_DINATALE_ENGLISH.pdf"}>CV <HiDownload /></a></li>
                <li className="hover:text-white duration-100 hover:bg-neutral-700 px-2 rounded-xl"><a target="_blank" href={"https://github.com/Dantelo04"} className="inline-flex justify-center items-center gap-2">Proyects <FaGithub /></a></li>
            </ul>
